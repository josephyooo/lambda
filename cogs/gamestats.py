...
class Gamestats:
    ...
    async def csgostats(self, ctx, id: str=''):
        if id:
            ...
            if isid:
                try:
                    player_summary = get(f'http://api.steampowered.com/ISteamUser/GetPlayerSummaries/v0002/?key={steam_api_key}&steamids={id}').json()['response']['players']
                    stats = get(f'http://api.steampowered.com/ISteamUserStats/GetUserStatsForGame/v0002/?appid=730&key={steam_api_key}&steamid={id}').json()['playerstats']['stats']
                ...
                status_codes = {0: 'Offline', 1: 'Online', 2: 'Busy', 3: 'Away', 4: 'Snooze', 5: 'Looking to trade', 6: 'Looking to play'}
                user_avatar = player_summary['avatar']
                user_name = player_summary['personaname']
                user_status = status_codes[player_summary['personastate']]
                embed = Embed(title=f"{user_name} - {user_status}", description=f"{user_name}'s CSGO statistics", color=0x0000f7)
                embed.set_thumbnail(url=player_summary['avatarfull'])
                embed.add_field(name='Kills', value='{} total kills'.format(obtained_stats['total_kills']))
                embed.add_field(name='Deaths', value='{} total deaths'.format(obtained_stats['total_deaths']))
                embed.add_field(name='Kill/Death Ratio', value=obtained_stats['total_kills'] / obtained_stats['total_deaths'])
                embed.add_field(name='Time Played', value='~{} hours'.format(obtained_stats['total_time_played'] // 3600))
                embed.add_field(name='Number of MVPs', value='{} MVPs'.format(obtained_stats['total_mvps']))
